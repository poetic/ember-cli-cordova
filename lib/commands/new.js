...
module.exports = Command.extend({
  ...
  run: function() {
    var root = path.join(process.cwd(), this.options.dirName);
    var configPath  = path.join(root, '.ember-cdv');

    this.config = new Config(configPath, {
      root:          path.join(process.cwd(), this.options.dirName),
      name:          stringUtils.classify(this.args.name),
      modulePrefix:  stringUtils.dasherize(this.args.name),
      id:            this.args.id
    });
  ...
  setupEmber: function() {
    ...
    var updateIndexHtml    = require('../tasks/update-index-html')(this.config);

    return createProject().then(installBowerDeps).then(installNpmDeps).then(removeDefaultFiles).then(copyEmberTmpls).then(updateIndexHtml);
  },
  linkEnvironment: function() {
    ...
    return new Promise(function(resolve, reject){
      try {
        var root = self.config.get('root');

        self.config.delete('root');
        // Hack to not write root since I don't need it after project is
        // created
        // It must be written for Project.closest to work below
        self.config.flush();
        self.config.set('root', root)

        // TODO: this is duplication of the path listed in
        // tasks/build. I'd like to remove it
        var link = linkEnv(Project.closest(root), 'dist');
